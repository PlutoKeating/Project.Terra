...
桌面端同时显示三栏；移动端以 Palette / Canvas / Inspector 标签在三栏之间切换，默认显示 Canvas，保证主画布始终拥有完整视口宽度。

认证页支持注册、登录、发送密码恢复邮件，以及从 Supabase 恢复链接进入后的新密码设置。项目页头部搜索框按项目名称和描述实时过滤当前用户的项目。
