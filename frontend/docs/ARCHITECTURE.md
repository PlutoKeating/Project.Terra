...
`AuthPage.tsx` implements sign-up, sign-in, recovery-email requests, and password updates after a Supabase recovery callback. `HeaderRail.tsx` routes search queries into `ProjectsPage` and owns the notification/settings popovers.
...
At desktop breakpoints the canvas remains a three-column editor. Below the `md` breakpoint, Palette, Canvas, and Inspector become mutually exclusive full-width panels controlled by a mobile tab strip; Canvas is the default.
